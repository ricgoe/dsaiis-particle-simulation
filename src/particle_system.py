...
class ParticleSystem:
    ...
    def move_particles(self, skip_interaction: bool = False):
        ...
        colliding_pairs = self.check_collisions(extended_positions, ghost_indices = ghost_indices) # sorted from left to right and bottom to top
    ...
    def check_collisions(self, positions: np.ndarray, radius: float = None, ghost_indices = None, mode = 'collision'):
        """
        ...
        for i, j in candidate_pairs:
            ...
            if distance <= 2 * radius:
                ...
                real_i = ghost_indices[i - n] if i >= n else i
                real_j = ghost_indices[j - n] if j >= n else j
            
                if real_i != real_j:  
                    colliding_pairs.append(((real_i, real_j), distance, normal))
